...
|---------------|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `8`           | `MAX_THREADS `             | Number of threads to use for CPU & I/O bound tasks. Recommend matching CPU cores.                                                       |
| `30`          | `SCAN_INTERVAL`            | Minutes between media library scans. We rebuild the media library from Radarr and Sonarr every scan, so be cautious setting too low.    |
| `128`         | `SCAN_BATCH_SIZE`          | Number of items in the scan task to process at any given time. This should ideally be set to a multiple of your `MAX_THREADS` variable. |
| `60`          | `FASTRESUME_INTERVAL`      | How often (in minutes) to save fastresume data. *Setting this too low can negatively impact your disk performance.*                     |
| **NONE**      | `ANNOUNCE_IP`              | Rarely needed, advanced only. If for some reason your client is making requests from different IP than you appear to peers.             |
| `6881`        | `TORRENTING_PORT`          | Port accepting connections from other torrent clients. (Make sure to bind this to host and forward in router.)                          |
| `INFO`        | `LOG_LEVEL`                | Lowest log level to show in console. Can be `DEBUG`, `INFO`, `WARNING`, `ERROR`, or `CRITICAL` where `DEBUG` shows most amount of logs  |
| `False`       | `PURGE_UNTRACKED_TORRENTS` | Enable this to have the client purge any dangling torent files that aren't currently tracked in the database - this can be destructive. |
